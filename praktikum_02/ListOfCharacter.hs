...
{- type List of Char: [ ] atau [e o List] atau [List o e]
  ...
  -- DEFINISI DAN SPESIFIKASI KONKAT
  konkat :: [Char] -> [Char] -> [Char]
  {- konkat lc1 lc2  menerima masukan 2 buah list of character, misalnya lc1 dan lc2,
   yang masing-masing mungkin kosong, dan menghasilkan list baru yang merupakan penggabungan
    lc1 dengan lc2 (lc1 di awal). -}

  konkat lc1 lc2 
    | isEmpty lc1 && isEmpty lc2 = []
    | isEmpty lc1 = (head lc2) `konso` (konkat lc1 (tail lc2))
    | otherwise = (head lc1) `konso` (konkat (tail lc1) lc2)


  -- DEFINISI DAN SPESIFIKASI FUNGSI pajakMakan
  pajakMakan :: [Char] -> [Int] -> [Char]
  {- pajakMakan li harga  menerima list nama makanan bertipe char dan list harganya bertipe integer.
  ...
  pajakMakan li harga 
    | isEmpty li = []
    | otherwise =
      if (head harga) * 11 <= 200 * 10 then head li `konso` pajakMakan (tail li) (tail harga)
      else pajakMakan (tail li) (tail harga)
